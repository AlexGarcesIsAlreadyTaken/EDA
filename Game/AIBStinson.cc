 ...
 */
#define PLAYER_NAME BStinson

#define MAX_SEARCHERS 3	//Max # of units will chase the same food
#define MAX_DISTANCE 20	//Max distance we will BFS
...
struct PLAYER_NAME : public Player {
   ...
   */
	...
	void bfs_find_units(const Pos& P, map<int, Movement>& M) {
		...
		if (board[P.i][P.j] == Walker) interest = 4;
		if (board[P.i][P.j] == Enemy) interest = 8;
		if (board[P.i][P.j] == Corpse) interest = 12;
				...
				if (distance <= DISTANCE and can_move(aux) and not visited[i][j]) {
					...
					if (object_in(aux, Ally) and not object_adjacent(p, Walker)) {
						if (units_moved.find(id) == units_moved.end()) {
							Movement m = {distance+interest, inverse(d), P};
							if (M.find(id) == M.end()) M.insert(make_pair(id, m));
							else if (M[id].distance > distance+interest) M[id] = m;
	...
	void wait_if_worth(int id) {
		...
		//Waits if finds 
		for (int i = 0; i < 4; ++i) {
			if (object_in(p+Dirs[i]+Dirs[i], Walker) and not (healthy(id) and object_adjacent(p+Dirs[i], Walker))) {
				units_moved.insert(id);
		} }
